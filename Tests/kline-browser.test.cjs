...
test('GPA chart stays readable under repeated zoom, pan, aggregation and refresh', { skip: !process.env.PLAYWRIGHT_MODULE }, async () => {
  ...
  try {
    ...
    state.gradeHistory = [3.67, 3.33, 3.71, 3.71].map((value, i) => ({ value, date: '2026-09-' + (18 + i), capturedAt: `2026-09-${18 + i}T08:00:00Z`, count: 7, scale: 4 }));
    ...
    for (const step of ['-1', '1']) {
      for (let i = 0; i < 20; i++) await page.locator('[data-action="gpa-kline-zoom"][data-step="' + step + '"]').click();
    ...
    state.gradeHistory = Array.from({ length: 120 }, (_, i) => {
      ...
      return { capturedAt, date: capturedAt.slice(0, 10), value: 3 + (i % 20) / 20, count: 7, scale: 4 };
    });
