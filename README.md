...
| ManageBac | 读取课程总评、任务、明确的教师反馈及原页入口；若已登录页面显示百分比权重的成绩组成，可在成绩页估算剩余学期 GPA | 学期起止日期需按学校校历在本机设置；未评分部分按已评分组成均分延续，课程等权，仅为情景估算。学生端看不到权重或成绩组成时不会伪造预测；不猜官方 GPA；需配置学校入口 |
