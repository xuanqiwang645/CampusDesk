...
(function (root, factory) {
  ...
  function create(options) {
    ...
    function summary(inputCourses) {
      ...
      return '<div class="gp-results"><div><small>' + L('情景参考 GPA', 'Scenario GPA') + '</small><strong>' + (report.gpa === null ? '—' : report.gpa.toFixed(2) + ' / 4.00') + '</strong><span>' + L('按已纳入课程等权计算', 'Equally weighted across included courses') + '</span></div><div><small>' + L('数据覆盖', 'Model coverage') + '</small><strong>' + report.count + ' / ' + report.rows.length + '</strong><span>' + L('门课程 · 缺失课程不计作零分', 'courses · missing courses are not scored as zero') + '</span></div></div><div class="gp-forecast-list">' + report.rows.map(row => '<article><div><strong>' + esc(row.course.name) + '</strong><small>' + (row.projectedPercent === null ? reasonText(row) : (row.source === 'manual' ? L('本机类别', 'Local categories') : 'ManageBac') + ' · ' + L('预测权重 ', 'Estimated weight ') + pct(row.adjustableWeight)) + '</small></div><b>' + pct(row.projectedPercent) + (row.projectedPercent !== null ? '<small>GPA ' + Core.gradePoints(row.projectedPercent).toFixed(2) + '</small>' : '') + '</b><button class="button button-light" type="button" data-gp-action="edit" data-gp-course="' + esc(key(row.course)) + '">' + L('调整', 'Adjust') + '</button></article>').join('') + '</div><p class="gp-footnote">' + L('显示本机情景估算，采用 90/80/70/60 → 4/3/2/1 的参考换算，不含学分或 AP 加权。日期仅显示日历进度，不代表已完成的评分权重。', 'Local scenario estimate using the reference 90/80/70/60 → 4/3/2/1 conversion, without credits or AP weighting. Dates show calendar progress, not completed grading weight.') + '</p>';
    }
