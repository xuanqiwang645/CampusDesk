(function () {
  ...
  function renderHistory() {
    ...
    const candles = history.map((h, index) => {
      ...
      const label = history.length <= 16 || index % Math.ceil(history.length / 12) === 0 ? lastUpdated(h.capturedAt || h.date) : '';
      return '<div class="gpa-kline-candle" style="position:relative;flex:0 0 38px;height:260px" title="' + esc(lastUpdated(h.capturedAt || h.date) + ' · 开 ' + open.toFixed(2) + ' · 高 ' + topValue.toFixed(2) + ' · 低 ' + bottomValue.toFixed(2) + ' · 收 ' + close.toFixed(2) + ' · ' + (h.count || 0) + ' 门课程') + '"><span style="position:absolute;left:18px;top:' + wickTop.toFixed(2) + '%;height:' + wickHeight.toFixed(2) + '%;width:2px;background:' + color + '"></span><span style="position:absolute;left:10px;top:' + bodyTop.toFixed(2) + '%;height:' + bodyHeight.toFixed(2) + '%;width:18px;min-height:8px;border-radius:3px;background:' + color + ';box-shadow:0 1px 2px rgba(40,65,52,.12)"></span><small style="position:absolute;left:-5px;right:-5px;bottom:-27px;text-align:center;color:#8b9884;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + esc(label) + '</small></div>';
    }).join('');
    return '<section class="card" style="margin-top:21px">' + cardHeader('clock', '本机 GPA K 线', '<span class="card-kicker">全部历史 · ' + history.length + ' 次</span>') + '<div style="display:flex;align-items:stretch;gap:10px;margin-top:8px"><div style="width:42px;flex:0 0 42px;height:260px;display:flex;flex-direction:column;justify-content:space-between;color:#8b9884;font-size:10px;text-align:right;padding:2px 0 0"><span>' + maxValue.toFixed(2) + '</span><span>' + midpoint.toFixed(2) + '</span><span>' + minValue.toFixed(2) + '</span></div><div style="overflow-x:auto;overflow-y:hidden;flex:1;padding:0 8px 34px 0"><div style="position:relative;display:flex;align-items:stretch;min-width:' + Math.max(600, history.length * 38) + 'px;height:260px;background:linear-gradient(to bottom,rgba(125,151,132,.22) 1px,transparent 1px,transparent calc(50% - 1px),rgba(125,151,132,.16) 50%,transparent calc(50% + 1px),transparent calc(100% - 1px),rgba(125,151,132,.22) calc(100% - 1px))">' + candles + '</div></div></div><div style="display:flex;gap:18px;flex-wrap:wrap;margin-top:20px;color:#778779;font-size:11px"><span><i style="display:inline-block;width:9px;height:9px;border-radius:2px;background:' + palette.up + ';margin-right:5px"></i>上涨</span><span><i style="display:inline-block;width:9px;height:9px;border-radius:2px;background:' + palette.down + ';margin-right:5px"></i>下降</span><span>当前设置：' + palette.label + '</span><span>开盘=上一条记录 · 收盘=当前 GPA</span></div><p class="gpa-note">每条 K 线对应一次本机 GPA 记录；数据源只提供离散 GPA，因此最高/最低取开盘与收盘值。悬停查看完整记录与计入课程数。</p></section>';
  }
